...
export class ClockComponent {
  ...
  constructor() {
    ...
    this.hours$ = this.time$.pipe(
      map((t) => t.getUTCHours().toString().padStart(2, '0'))
    );
    this.minutes$ = this.time$.pipe(
      map((t) => t.getUTCMinutes().toString().padStart(2, '0'))
    );
    this.seconds$ = this.time$.pipe(
      map((t) => t.getUTCSeconds().toString().padStart(2, '0'))
    );
  }
}
